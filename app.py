...
# LAYOUT
app.layout = html.Div([
    html.H1('Jorge Marmol - Dashboard Social Networks', style={
    ...
    html.H3("Avg. Visit Duration"),
    html.H2(id='avg-visit-duration'),
...
@app.callback(
    ...
    Output('avg-visit-duration', 'children'),
...
def update_unique_users(start_date, end_date, social_networks, devices):
    filtered_df = df[
        (df['date'] >= start_date) &
        (df['date'] <= end_date) &
        (df['social_network'].isin(social_networks)) &
        (df['device'].isin(devices))
    ]
    unique_users = filtered_df['user_id'].nunique()
    return str(unique_users)

def update_avg_visit_duration(start_date, end_date, social_networks, devices):
    filtered_df = df[
        (df['date'] >= start_date) &
        (df['date'] <= end_date) &
        (df['social_network'].isin(social_networks)) &
    ...
    avg_duration = filtered_df['visit_duration'].mean()
    return f"{avg_duration:.2f} minutes"
